...
int mix_nvm_init(){
    ...
    nvm_info->block_num = 1024*1024*1024;//10GB
...
static inline void mix_ntstore64(unsigned long* dst,unsigned long n){
...
static inline void mix_ntstorenx64(const void* dst, const void* src, size_t len){
    for(size_t i = 0; i < len; i = i + 8){
        mix_ntstore64((unsigned long*)(dst + i),*(unsigned long*)(src + i));
    }
}
