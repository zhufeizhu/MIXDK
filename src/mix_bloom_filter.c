...
static void hash_func(mix_counting_bloom_filter_t* bloom_filter, int key, __uint32_t* hashes){
    int i;
...
int mix_counting_bloom_filter_add(mix_counting_bloom_filter_t* bloom_filter, uint32_t key){
    ...
    for(i = 0; i< bloom_filter->nfuncs; i++){
        ...
        mix_counting_bitmap_increment(bloom_filter->bitmap, index, bloom_filter->offset);
    }
...
int mix_counting_bloom_filter_remove(mix_counting_bloom_filter_t* bloom_filter,uint32_t key){
    ...
    for(i = 0; i < bloom_filter->nfuncs; i++){
        offset = i * bloom_filter->counts_per_func;
        index = hashes[i] + offset;
        mix_counting_bitmap_decrement(bloom_filter->bitmap, index, bloom_filter->offset);
    }
    bloom_filter->header->count--;
...
int mix_counting_bloom_filter_test(mix_counting_bloom_filter_t* bloom_filter, uint32_t key){
    ...
    for(i = 0; i < bloom_filter->nfuncs; i++){
        offset = i*bloom_filter->counts_per_func;
        index = hashes[i] + offset;
        if(!mix_counting_bitmap_check(bloom_filter->bitmap,index, bloom_filter->offset)){
            return 0;
        }
    }
