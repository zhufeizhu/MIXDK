...
static unsigned __kfifo_max_r(unsigned int a, unsigned int b)
{
     return (a > b) ? a : b;
}
...
unsigned int __kfifo_out_peek_offset(struct __kfifo *fifo,
...
int main(int argc, char *argv[])
{
     ...
     fifo_buf = malloc((1024 * 1024));

     (
         {
              typeof((&fifo) + 1) __tmp = (&fifo);
              struct __kfifo *__kfifo = &__tmp->kfifo;
              (sizeof(*__tmp) == sizeof(struct __kfifo)) ? __kfifo_init(__kfifo, fifo_buf, (1024 * 1024), sizeof(*__tmp->type)) : --1;
         });

     len = ((&fifo)->kfifo.mask + 1);
     ...
     (
         {
              ...
              unsigned long __n = (strlen(buf) + 1);
     ...
     len = __kfifo_uint_must_check_helper((
         {
              typeof((&fifo) + 1) __tmpq = (&fifo);
              const size_t __recsize = sizeof(*__tmpq->rectype);
              unsigned int __avail = ((__tmpq)->kfifo.mask + 1) - (
                                                                      {
